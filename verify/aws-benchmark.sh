#!/usr/bin/env bash
#
# aws-benchmark.sh — Verification benchmark: Latte vs YCSB on Alternator.
#
# Confirms three hypotheses from explore/:
#   H1: Latte sustains ~30% higher peak throughput than tuned-YCSB
#   H2: Latte tail latency is ~3-5× tighter at matched submitted rate
#   H3: The gap is a constant percentage (scales with hardware)
#
# Usage:
...
iu99 = g(r'\[Intended-UPDATE\], 99thPercentileLatency\(us\), ([0-9.]+)') / 1000
...
if total > 0:
    ...
    agg_req_mean = 0; agg_req_p99 = 0

print(f'ycsb,$inflight,$rate_str,$rep,{tp:.1f},{get_cycle_mean:.3f},{get_cycle_p99:.3f},{upd_cycle_mean:.3f},{upd_cycle_p99:.3f},{agg_req_mean:.3f},{agg_req_p99:.3f},$loader_cpu,$scylla_cpu,$scylla_ops,$scylla_p99,$scylla_reactor')
"
        fi
...
ga = get_m('get'); g99 = get_p('get', p99i)
ua = get_m('update'); u99 = get_p('update', p99i)
...
print(f'latte,$inflight,$rate_str,$rep,{tp:.1f},{ga:.3f},{g99:.3f},{ua:.3f},{u99:.3f},{rl_mean:.3f},{rl_p99_val:.3f},$loader_cpu,$scylla_cpu,$scylla_ops,$scylla_p99,$scylla_reactor')
"
        fi
    fi
}
...
validate_smoke() {
    local phase_dir="$1"
    local ycsb_log="$phase_dir/ycsb/inflight=32/rep1/ycsb_1.log"
    ...
    if ! grep -q '\[Intended-READ\]' "$ycsb_log"; then
        ...
        echo "Check benchmarks/run-ycsb-benchmark.sh for the -p measurement.interval=both line."
        echo "Log: $ycsb_log"
        echo
    ...
    local prom_log="$phase_dir/ycsb/inflight=32/rep1/scylla_prometheus.log"
